...
- **Meta (Instagram/Facebook/Threads):** one Meta app covers all three.
  ...
  need a public `media_url` — the engine uses the photos already hosted on the
  website (`media:` in `config/brand.yaml`). App Review is **not** needed to
  publish to accounts you administer while the app is in Development mode —
  step-by-step in `docs/social-engine.md`.
