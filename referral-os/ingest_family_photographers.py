#!/usr/bin/env python3
"""
INGEST — 30A family / vacation photographers.

Why 30A-only (not drive markets)
--------------------------------
Unlike wedding planners, family photographers don't "feed" 30A from other
cities. A family photographer based in Atlanta shoots Atlanta families —
they're not sending clients to the Emerald Coast. The ones useful to us
are the 30A-based photographers who shoot vacationing families on the
beach and get asked "where should we stay?" by every single client.

So this script searches *only* 30A metros, but uses THREE query angles
per metro to cast a wider net (Google Places weighs query terms heavily,
so different phrasings surface different photographers):

  • "family photographer in {metro}"     — general family shoots
  • "beach photographer in {metro}"      — beach-session specialists
  • "vacation photographer in {metro}"   — explicitly vacation-oriented

All three feed the same `photographer-family` segment, deduped by
google_place_id.

Run it
------
    cd "/Users/samantha/Documents/Claude/Projects/Sun & Moon at 30a/referral-os"
    python3 ingest_family_photographers.py
"""
...
SEGMENT_SLUG = "photographer-family"
...
# Three query angles per metro — each surfaces a different cluster of photographers
QUERY_TEMPLATES = [
    "family photographer in {metro}",
    "beach photographer in {metro}",
    "vacation photographer in {metro}",
]
...
def place_to_company(p: dict, query_angle: str) -> dict:
    ...
    return {
        ...
        "metadata": {
            ...
            "query_angle":   query_angle,   # which phrase surfaced this one
            "market_role":   "local",
            "market_metro":  "30a",
        },
    }
...
def main() -> int:
    ...
    angle_used: dict[str, str] = {}   # which angle first surfaced each place
    for metro in METROS:
        per_metro_added = 0
        for template in QUERY_TEMPLATES:
            q = template.format(metro=metro)
            angle = template.split(" photographer")[0]
            try:
                data = google_places_search(google_key, q)
            except urllib.error.HTTPError as e:
                print(f"  {metro:25s} [{angle:9s}]: HTTP {e.code}")
                continue
            except Exception as e:  # noqa: BLE001
                print(f"  {metro:25s} [{angle:9s}]: ERROR {e}")
                continue
            ...
            print(f"  {metro:25s} [{angle:9s}]: {len(places):2d} returned, {new_here:2d} new")
            time.sleep(0.2)
        print(f"  → {metro:25s} subtotal: {per_metro_added} new\n")

    print(f"Unique photographers found: {len(by_pid)}")
    if not by_pid:
    ...
    print()
    print("=" * 70)
    print("RESULTS — 30A family / beach / vacation photographers")
    print("=" * 70)
