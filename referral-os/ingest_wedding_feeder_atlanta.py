#!/usr/bin/env python3
"""
DRIVE-MARKET INGEST — wedding planners in Atlanta.

Pivot: Apollo's search endpoints are gated to Professional ($99/mo). So we
use Google Places — same proven plumbing as ingest_wedding_planners.py —
just pointed at Atlanta metros instead of 30A, and linked to the
`wedding-planner-feeder` segment.

What we're after
----------------
Wedding planners in Atlanta who book *destination weddings on 30A.*
We don't know that for certain just from a Places listing — that's a
hand-qualification step later. But getting the list of 50+ Atlanta
planners on the table is step one.

Why Atlanta specifically
------------------------
~5-hour drive to 30A. Largest feeder metro for the Emerald Coast
destination-wedding market. If this pull comes back clean, we expand
to Birmingham, Nashville, New Orleans next.

Run it
------
    cd "/Users/samantha/Documents/Claude/Projects/Sun & Moon at 30a/referral-os"
    python3 ingest_wedding_feeder_atlanta.py
"""
...
# Atlanta metro is huge. Searching a few specific neighborhoods + suburbs
# yields different result sets than just "Atlanta GA" — Google Places
# weighs proximity heavily, so each query surfaces different planners.
METROS = [
    "Atlanta GA",
    "Buckhead Atlanta GA",      # upscale, where many destination-wedding planners cluster
    "Marietta GA",
    "Alpharetta GA",
    "Decatur GA",
    "Sandy Springs GA",
...
def place_to_company(p: dict) -> dict:
    ...
    return {
        ...
        "metadata": {
            ...
            "market_role":   "feeder",  # tag so we can distinguish from local 30A planners later
...
def main() -> int:
    ...
    # --- 2. search google places across atlanta metros ---
    print(f"\nSearching Google Places across {len(METROS)} Atlanta-area locations ...")
    ...
    for metro in METROS:
        ...
        for p in places:
            if p["id"] not in by_pid:
                ...
                by_pid[p["id"]] = p
                added += 1
        print(f"  {metro:30s}: {len(places):3d} returned, {added:3d} new")
        time.sleep(0.2)
    ...
    links = [
        {
            ...
            "relevance_score": 70,  # slightly lower than local (75) — feeders need hand-qualification
            "status":          "prospect",
        }
        for c in upserted
    ]
    ...
    print()
    print("=" * 70)
    print("RESULTS — Atlanta wedding-planner feeders")
    print("=" * 70)
    ...
    print()
    print("Top 10 by Google review count (best signal of who's active):")
    sorted_rows = sorted(
        rows,
    ...
    for r in sorted_rows[:10]:
        ...
        print(f"  ★ {rating}  ({count:>4} reviews)  {r['name'][:45]:45s}  {site}")
    print()
    print("👉 Open Supabase → SQL Editor and run:")
    print("   SELECT name, city, state, website, metadata->>'rating' AS rating,")
    print("          metadata->>'rating_count' AS reviews")
    print("   FROM companies")
    print("   WHERE metadata->>'market_metro' = 'atlanta'")
    print("   ORDER BY (metadata->>'rating_count')::int DESC NULLS LAST;")
    return 0
